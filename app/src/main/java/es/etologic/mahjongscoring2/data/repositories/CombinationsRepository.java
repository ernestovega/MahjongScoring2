...
public class CombinationsRepository extends BaseRepository<Combination> { /*TODO Hacer pruebas forzando datos (tests unitarios!)*/
    ...
    public List<Combination> getAll() {
        List<Combination> combinations = combinationsDao.getAllOrderedAscByPoints();
        if(combinations.isEmpty()) {
            combinationsDao.bulkInsert(getAllHardcodedCombinations());
            combinations = combinationsDao.getAllOrderedAscByPoints();
        }
        return combinations;
    }
    ...
    @Override
    public long deleteAll() { return combinationsDao.deleteAll(); }
}
